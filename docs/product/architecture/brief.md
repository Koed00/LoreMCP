...
- **Tool contracts** (informational -- exact JSON Schema is a software-crafter implementation detail, but the SHAPES below are binding per shared-artifacts-registry.md):
  - `list_features(repo_name: string)` -> `{repo_name, doc_path, features: [{feature_id, phases: string[]}], has_architecture_adrs: boolean, has_claude_md: boolean}` | or one of the structured error shapes. **(tool-output-quality, slice 03)** `phases` semantics for the `"deliver"` entry: included if AND ONLY IF `docs/feature/{feature_id}/deliver/execution-log.json` exists, is parseable JSON, has an `events` array, AND at least one element has `p === "COMMIT"` -- mere existence of the `deliver/` directory is NOT sufficient (this was the defect: `wave-decisions.md`-presence detection, used for design/discuss/distill, does not apply to deliver since DELIVER produces no such file). Malformed/unparseable `execution-log.json` is treated as "deliver not detected" (fail-closed, no new warning). All other phase entries (`design`, `discuss`, `discover`, `distill`) are unaffected -- still detected via `wave-decisions.md` presence, unchanged.
  - `query_context(repo_name: string, feature_id: string)` -> `{repo_name, feature_id, results: [{source_file, phase, snippet}], retrieved_at: string, warnings?: string[]}` | or one of the structured error shapes. **(tool-output-quality, slice 01, ADR-007)** `results` is additionally capped at a TOTAL-response budget (`TOTAL_RESPONSE_MAX_CHARS = 24000`, 3x the existing per-file `SNIPPET_MAX_CHARS`) across all results combined -- when exceeded, the OLDEST results (by wave/phase order) are dropped whole until the kept (most-recent-first) subset fits, and a `warnings` entry is added (e.g. `"Response truncated to stay within the total size limit; oldest wave content dropped. {N} of {M} results omitted."`). This is independent of, and composes with, the existing per-file truncation (ADR-003) -- a normal-sized feature (1-3 phases) is unaffected.
...
- `adr-007-total-response-truncation-strategy.md` -- result-array-level truncation (drop-oldest-whole) for `query_context`'s new total-response cap, rejecting reuse of `capSnippetAtHeadingBoundary` on a concatenated string (`tool-output-quality` feature, slice 01). No new ADR for slices 02-04 (`tool-output-quality`) -- each is a single-file pattern-following extension or string edit with no genuine architecture-level alternative; see `docs/feature/tool-output-quality/design/wave-decisions.md`.
...
- [x] tool-output-quality (DESIGN wave, regression-recovery round): 4 independent defect fixes, no new tool/component/topology change; Reuse Analysis per slice in `docs/feature/tool-output-quality/design/architecture-design.md`; new ADR-007 for slice 01's truncation-mechanism decision (genuine alternatives); no ADR for slices 02-04 (pattern-following extensions or string edits, no genuine alternative) per `docs/feature/tool-output-quality/design/wave-decisions.md`; Section 8 contract notes updated for `query_context` (total-response cap) and `list_features` (`deliver` phase semantics); C4 diagrams confirmed unchanged (no new component/tool)
